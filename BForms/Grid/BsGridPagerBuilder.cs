...
{
    public class BsGridPagerBuilder : BaseComponent
    {
        #region Properties and Constructor
        private BsPagerModel pager { get; set; }
        private BsPagerSettings settings { get; set; }
        private BsGridBaseRepositorySettings baseSettings { get; set; }
        ...
        public override string Render()
        {
                ...
                if (this.pager == null || this.pager.TotalRecords == 0)
                {
                ...
                if (this.settings.HasPageSizeSelector)
                {
                    int pageSize = this.pager != null ? this.pager.PageSize : this.baseSettings.PageSize;
                    if (!this.settings.PageSizeValues.Contains(pageSize))
                    ...
                    foreach (var item in this.settings.PageSizeValues)
                    {
        ...
        public string RenderPages()
        {
            if (this.pager != null)
            {
                ...
                if (this.settings.ShowPrevNextButtons)
                {
                    ...
                    var anchorBuilder = new TagBuilder("a");
                    anchorBuilder.MergeAttribute("href", "#");
                    anchorBuilder.MergeAttribute("data-page", (pager.CurrentPage - 1).ToString());
                ...
                var startPage = this.pager.GetStartPage(this.settings.Size);
                int nr = this.settings.Size > this.pager.TotalPages ? this.pager.TotalPages % this.settings.Size : this.settings.Size;
                for (int i = 0; i < nr; i++)
                {
                ...
                if (this.settings.ShowPrevNextButtons)
                {
                    ...
                    var anchorBuilder = new TagBuilder("a");
                    anchorBuilder.MergeAttribute("href", "#");
                    anchorBuilder.MergeAttribute("data-page", (this.pager.CurrentPage + 1).ToString());
                ...
                if (this.settings.ShowFirstLastButtons)
                {
                    ...
                    var anchorBuilder = new TagBuilder("a");
                    anchorBuilder.MergeAttribute("href", "#");
                    anchorBuilder.MergeAttribute("data-page", this.pager.TotalPages.ToString());
                ...
                if (this.settings.HasPagesText)
                {
                    var textBuilder = new TagBuilder("div");
                    textBuilder.MergeAttribute("class", "results_number");

                    var firstIdx = (this.pager.CurrentPage - 1) * this.pager.PageSize + 1;
                    var lastIdx = this.pager.CurrentPage == this.pager.TotalPages
                                      ? this.pager.TotalRecords
                                      : this.pager.CurrentPage * this.pager.PageSize;

                    var totalCountBuilder = new TagBuilder("span");
                    totalCountBuilder.InnerHtml += this.pager.TotalRecords;
